...
const FOOTBALL_PLAYERS = [
    ...
    "wayne rooney","didier drogba","samuel eto'o","sergio aguero",
    "eden hazard","mesut ozil","alexis sanchez","arturo vidal","ngolo kante",
    ...
    "hakim ziyech",
    "gabriel jesus","gabriel martinelli","richarlison","raphinha",
    ...
    "kalidou koulibaly","edouard mendy","victor osimhen",
    "khvicha kvaratskhelia","joao cancelo","ferran torres",
    "ansu fati","pedro","alvaro morata","gerard moreno","jordi alba","sergi roberto",
    "pepe","renato sanches","joao moutinho","ruben neves","diogo dalot",
    "nuno mendes","vitinha",
    "romelu lukaku",
    "zlatan ibrahimovic","edinson cavani","luis suarez","fernando torres",
    "david villa","raul","dennis bergkamp","yaya toure",
   "arsene wenger","pep guardiola","jurgen klopp"
];
...
socket.on('ready_to_launch', () => {
  log('Ready to launch');
  document.getElementById('lstat').textContent = '✅ Ready! (1+ players)';
});
...
// ✅ FIXED: Load themed pictures for Osteth (TheSportsDB works!)
async function loadOstethPictures(theme) {
  ...
  if (theme === 'football') {
    ...
    for (const player of selected) {
      try {
        ...
        if (data.player && data.player[0]) {
          ...
          pictures.push({
            // ✅ TheSportsDB URL (works in browser context!)
    ...
    for (let i = 0; i < 3; i++) {
      pictures.push({ url: `https://picsum.photos/400?random=${Date.now() + i}`, name: `Random ${i+1}`, detail: 'Mystery Picture', apiSource: 'direct' });
    }
  }
  
  // ✅ Load images with proper CORS handling
  for (let i = 0; i < 3; i++) {
    ...
    if (img && pictures[i]) {
      ...
      // ✅ Load image (browser handles CORS when displayed in img tag)
...
// ✅ Upload image - handles both canvas and direct URL
async function uploadImageForOsteth(imgElement) {
  ...
  try {
    // ✅ For TheSportsDB and external URLs - send URL to server to download
    ...
    // ✅ For avatar/canvas - use blob upload
...
document.getElementById('conf-ost').onclick = async () => {
  ...
  try {
    ...
    ostethExpectedRound = document.getElementById('exp-r').value;
